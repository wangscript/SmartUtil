...
public class CsvUtil {
	...
	public static String toCsvString(Object... elements) {
		...
		for (int i = 0; i < elements.length; i++) {
			...
			if (ndx == -1) {
				if (field.startsWith(Constant.SPACE) || field.endsWith(Constant.SPACE)) {
					ndx = 1;
				}
			}
			if (ndx == -1) {
				ndx = StringHelper.indexOfChars(field, SPECIAL_CHARS);
			}
			...
			field = StringHelper.replace1(field, Constant.QUOTE, DOUBLE_QUOTE);
			line.append(field);
			if (ndx != -1) {
				line.append(FIELD_QUOTE);
			}
	...
	private static void addField(List<String> row, String line, int startIndex, int endIndex, boolean inQuoted) {
		...
		if (inQuoted) {
			field = StringHelper.replace1(field, DOUBLE_QUOTE, "\"");
		}
        row.add(field);
    }
