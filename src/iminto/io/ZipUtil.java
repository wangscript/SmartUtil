...
public class ZipUtil {
	...
	public static void addToZip(ZipOutputStream zos, File file, String path, String comment, boolean recursive) throws IOException {
		...
		if (isDir) {
			// add folder record
			if (!StringHelper.endsWithChar(path, '/')) {
				path += '/';
			}
		}
		...
		if (recursive && file.isDirectory()) {
			boolean noRelativePath = StringHelper.isEmpty(path);

			final File[] children = file.listFiles();

			if (children != null && children.length != 0) {
				for (File child : children) {
					String childRelativePath = (noRelativePath ? Constant.EMPTY : path) + child.getName();
